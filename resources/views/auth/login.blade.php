...
<div class="flex justify-center">
  <div class="w-4/12 p-6 bg-white rounded-lg">
    @if (session('status'))
    <div class="bg-red-500 text-white text-center rounded-lg p-4 mb-6">{{ session('status') }}</div>
    @endif
    <form action="{{route('login')}}" method="POST">
      ...
      <div>
        <button type="submit" class="bg-blue-600 w-full px-4 py-3 rounded-lg text-white font-medium">Login</button>
      </div>
    </form>
  </div>
</div>
